(function () {
  ...
  function isTargetUrl(url: any): boolean {
    if (!url) return false;
    let urlStr = '';
  ...
  function checkIsFirstPage(url: string | URL | RequestInfo): boolean {
    if (!url) return true;
  ...
  function extractFriends(body: any, url: any) {
    let friends = [];
    ...
    if (urlStr.includes('/followers/') && body.users && Array.isArray(body.users)) {
      ...
      friends = body.users.map((u: any) => ({ id: String(u.pk || u.id), name: u.username }));
    ...
    // Pattern 2: GraphQL (user -> edge_followed_by)
      ...
      if (connection.edges) {
        friends = connection.edges.map((e: any) => ({ id: String(e.node.id), name: e.node.username }));
      }
  ...
  XHR.open = function (_method: string, url: string | URL) {
    this._url = url;
